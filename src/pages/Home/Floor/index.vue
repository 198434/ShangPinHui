...
export default {
    ...
    mounted(){
        ...
        // 第一次书写轮播图的时候，是在当前组件内部发请求、动态渲染结构【前提至少服务器的数据需要回来】，因此当年的写法在这里不行
        // 现在的这种写法为什么可以：因为请求是父组件发的，父组件通过props传递过来，而且结构都已经有了的情况下执行mounted
        this.$nextTick(()=>{
            // 当你执行这个回调函数的时候：保证服务器数据回来了，v-for执行完毕了【一定轮播图的结构一定有了】
